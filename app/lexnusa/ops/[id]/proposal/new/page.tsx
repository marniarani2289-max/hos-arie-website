    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      ...
      <form action={createProposal} className="rounded-2xl bg-white p-6 text-[#0D1B2A] sm:p-8">
        ...
        <div className="grid gap-5">
          <label className="text-sm font-bold">Proposal title<input name="title" required maxLength={240} defaultValue={`${lead.project_type} — Pilot Proposal`} className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal"/></label>
          <label className="text-sm font-bold">Pilot scope<textarea name="scope" required maxLength={12000} rows={6} defaultValue={defaultScope} className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal leading-6"/></label>
          <label className="text-sm font-bold">Deliverables<textarea name="deliverables" required maxLength={12000} rows={6} defaultValue={defaultDeliverables} className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal leading-6"/></label>
          <label className="text-sm font-bold">Timeline<input name="timeline" required maxLength={4000} placeholder="Example: 7–10 business days from written confirmation and receipt of agreed materials." className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal"/></label>
          <div className="grid gap-4 sm:grid-cols-[1fr_120px_1fr]">
            <label className="text-sm font-bold">Proposal fee<input name="fee" required type="number" min="0" step="0.01" defaultValue={fee} className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal"/></label>
            <label className="text-sm font-bold">Currency<select name="currency" defaultValue={lead.currency || "USD"} className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 font-normal"><option>USD</option><option>IDR</option><option>SGD</option><option>MYR</option></select></label>
            <label className="text-sm font-bold">Valid until<input name="valid_until" type="date" className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal"/></label>
          </div>
          <label className="text-sm font-bold">Terms / notes<textarea name="terms" maxLength={12000} rows={5} defaultValue="This proposal is for a focused pilot engagement. Final scope, confidentiality arrangements, source access, turnaround, and any material scope changes must be agreed in writing. Fees exclude third-party costs unless expressly stated." className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal leading-6"/></label>
