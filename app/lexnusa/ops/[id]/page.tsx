...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen bg-[#0D1B2A] px-5 py-14 text-white sm:px-8"><div className="mx-auto max-w-6xl">
    <div className="flex flex-wrap items-start justify-between gap-5"><div><Link href="/lexnusa/ops" className="text-sm text-slate-300 underline">← Back to CRM</Link><p className="mt-5 text-xs font-black uppercase tracking-[.22em] text-[#C9A24B]">LEX-{lead.id} · {lead.project_type}</p><h1 className="mt-2 text-4xl font-black sm:text-5xl">{lead.name}</h1><p className="mt-3 text-slate-300">{lead.organization || "No organization"} · {lead.email}</p></div><div className="flex flex-wrap gap-3">{lead.status === "qualified" ? <Link href={`/lexnusa/ops/${lead.id}/proposal/new`} className="rounded-xl bg-[#0E6B6F] px-5 py-3 font-black text-white">Create Proposal</Link> : null}<form action={prepareFollowUpEmail}><input type="hidden" name="id" value={lead.id}/><button className="rounded-xl bg-[#C9A24B] px-5 py-3 font-black text-[#0D1B2A]">Email Client</button></form></div></div>
    ...
    <section className="mt-8 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
      ...
      <form action={updateLead} className="rounded-2xl bg-white p-6 text-[#0D1B2A]"><input type="hidden" name="id" value={lead.id}/><input type="hidden" name="return_to" value="/lexnusa/ops"/><h2 className="text-xl font-black">Lead Management</h2><div className="mt-5 grid gap-4">
