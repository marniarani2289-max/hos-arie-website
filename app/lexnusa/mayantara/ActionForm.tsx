...
export default function ActionForm({action,children}:{action:(state:FormState,form:FormData)=>Promise<FormState>;children:React.ReactNode}){
 ...
 return <form action={formAction} className="space-y-4"><fieldset disabled={pending} className="space-y-4 disabled:opacity-60">{children}</fieldset>{pending&&<p role="status">Menyimpan…</p>}{state.message&&<p role={state.ok?"status":"alert"} className="rounded-lg border p-3">{state.message}</p>}</form>;
}
