...
export async function requestSeat(_previous:FormState,form:FormData):Promise<FormState>{
 ...
 if(display_name.length<2||display_name.length>120||student_number.length<2||student_number.length>50) return {ok:false,message:"Masukkan nama lengkap dan NIM."};
 const {error}=await db.from("lexnusa_mayantara_members").insert({user_id:user.id,display_name,student_number});
 if(error)return {ok:false,message:"Pengajuan tidak dapat disimpan. Muat ulang halaman untuk memeriksa apakah Anda sudah mengajukan keanggotaan."};
 revalidatePath(roomPath);return {ok:true,message:"Pengajuan tersimpan. Dosen perlu mengonfirmasi keanggotaan kelas Anda."};
}
export async function setMemberStatus(_previous:FormState,form:FormData):Promise<FormState>{
 ...
 if(!["active","suspended"].includes(status))return {ok:false,message:"Status tidak valid."};
 const {error}=await db.from("lexnusa_mayantara_members").update({status}).eq("user_id",field(form,"student_id"));
 if(error)return {ok:false,message:"Keanggotaan tidak dapat diperbarui."};
 revalidatePath(roomPath);revalidatePath("/lexnusa/mayantara/lecturer");
 return {ok:true,message:status==="active"?"Mahasiswa diterima.":"Akses pengumpulan ditangguhkan."};
}
export async function saveWork(previous:FormState,form:FormData):Promise<FormState>{
 ...
 if(!getAssignment(assignment)||!["save","submit"].includes(intent)||answer.length>30000||sources.length>12000||reflection.length>8000)return fail("Periksa tugas dan panjang isian.");
 if(submitted&&(!attested||answer.length<200||sources.length<40||reflection.length<40))return fail("Pengumpulan memerlukan sedikitnya 200 karakter analisis, 40 karakter sumber, 40 karakter refleksi, dan pernyataan integritas Anda.");
 ...
 if(error||!data)return fail(error?.message.includes("stale_version")?"Ada versi yang lebih baru. Salin perubahan Anda sebelum memuat ulang halaman, lalu bandingkan dengan versi terbaru.":"Tugas tidak dapat disimpan. Teks Anda tetap di sini; periksa keanggotaan dan coba lagi.");
 revalidatePath(roomPath);revalidatePath("/lexnusa/mayantara/lecturer");
 return {ok:true,message:submitted?"Versi dikumpulkan untuk ditinjau dosen.":"Versi draf tersimpan.",versionId:data.id};
}
export async function reviewWork(_previous:FormState,form:FormData):Promise<FormState>{
 ...
 if(rubric.some(r=>!Number.isInteger(scores[r.key])||scores[r.key]<0||scores[r.key]>r.max)||feedback.length<40||feedback.length>10000||!["reviewed","revise"].includes(decision))return {ok:false,message:"Masukkan nilai setiap kriteria rubrik dan sedikitnya 40 karakter umpan balik."};
 ...
 if(loadError||!work||work.user_id===user.id)return {ok:false,message:"Tugas ini tidak dapat ditinjau oleh akun ini."};
 ...
 if(error)return {ok:false,message:"Tinjauan tidak tersimpan. Muat ulang halaman: hanya versi terkini yang sudah dikumpulkan yang dapat menerima satu tinjauan."};
 revalidatePath(roomPath);revalidatePath("/lexnusa/mayantara/lecturer");
 return {ok:true,message:"Tinjauan tercatat untuk versi ini."};
}
