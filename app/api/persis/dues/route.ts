...
export async function POST(request: Request) {
  ...
  try { await linkCurrentMember(); } catch { return Response.json({error:'Data anggota belum dapat diperiksa. Silakan coba kembali atau hubungi bendahara.'},{status:503}); }
  ...
  const {data:existing,error:lookupError}=await supabase.from('persis_dues').select('id').eq('user_id',user.id).eq('period',values.period).eq('status','pending').limit(1);
  if(lookupError) return Response.json({error:'Catatan belum dapat diperiksa. Silakan coba kembali.'},{status:503});
  if(existing?.length) return Response.json({error:'Masih ada pengajuan bulan ini yang menunggu verifikasi. Tunggu hasilnya sebelum mencatat pembayaran tambahan.'},{status:409});
