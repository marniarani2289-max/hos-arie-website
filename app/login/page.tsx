...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string; next?: string; success?: string }> }) {
  ...
  const persis = next?.startsWith("/persis-kepri/");
  return <main className="min-h-screen bg-[#070916] px-5 py-20 text-white"><section className="mx-auto max-w-lg border border-white/15 bg-[#111526] p-8 sm:p-12">
    <p className="text-xs font-bold uppercase tracking-[.28em] text-amber-400">{persis ? "Iuran anggota PW Persis Kepri" : "Learning platform"}</p><h1 className="mt-4 text-4xl font-black">Masuk</h1>
    ...
    <form action="/auth/sign-in" method="post" className="mt-8 grid gap-5">
      ...
      <button className="bg-amber-400 px-6 py-4 font-bold text-slate-950 hover:bg-amber-300">{persis ? "Masuk ke iuran Persis" : next?.startsWith("/lexnusa/cohort")?"Masuk ke kelas LexNusa":"Masuk ke dashboard"}</button>
    </form>
    <div className="mt-7 flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-300">
      <p>Belum punya akun? <Link href={persis ? "/persis-kepri/daftar" : next?.startsWith("/lexnusa/cohort")?"/lexnusa/cohort/register":"/register"} className="font-bold text-amber-400">Daftar</Link></p>
