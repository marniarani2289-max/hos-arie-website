...
export async function registerPersis(form:FormData){
  ...
  const {data,error}=await supabase.auth.signUp({email,password,options:{emailRedirectTo:`${site}/auth/callback?next=/persis-kepri/iuran`,data:{full_name:name,institution:'PW Persis Kepulauan Riau'}}});
  ...
  if(data.session)redirect('/persis-kepri/iuran');
