...
export default async function RegisterPersis({searchParams}:{searchParams:Promise<{error?:string;success?:string}>}){
  ...
  return <div lang="id" className="min-h-screen bg-slate-50 px-5 py-12 text-slate-900"><section className="mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-6 sm:p-9"><Link href="/persis-kepri/iuran" className="text-sm font-semibold text-emerald-800 underline">Kembali ke iuran anggota</Link><p className="mt-6 text-sm font-bold text-emerald-800">PW Persis Kepulauan Riau</p><h1 className="mt-2 text-3xl font-bold">Buat akun anggota</h1><p className="mt-4 leading-7 text-slate-600">Sudah memiliki akun di website ini? Gunakan akun yang sama untuk masuk. Setelah verifikasi email, masuk dan lengkapi data anggota untuk diperiksa bendahara.</p>
    ...
    <Link href="/login?next=%2Fpersis-kepri%2Fpendaftaran" className="mt-6 inline-block font-semibold text-emerald-800 underline underline-offset-4">Masuk dengan akun yang sudah ada</Link>
  </section></div>;
}
