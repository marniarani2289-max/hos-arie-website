...
export default function CashForm({id,today,startDate}:{id:string;today:string;startDate:string}){
 const [kind,setKind]=useState<'expense'|'income'>('expense'),[busy,setBusy]=useState(false),[error,setError]=useState('');
 ...
 return <form onSubmit={submit} className="mt-5 grid gap-5"><input type="hidden" name="id" value={id}/>
  <fieldset disabled={busy} className="grid gap-5 sm:grid-cols-2">
   ...
   <label className="grid gap-2 text-sm font-semibold">Tanggal transaksi<input name="transacted_on" type="date" min={startDate} max={today} defaultValue={today} required className={input}/></label>
   ...
   <label className="grid gap-2 text-sm font-semibold sm:col-span-2">Kegiatan / program kerja (opsional)<input name="activity" maxLength={160} placeholder="Contoh: Pembinaan dai wilayah" className={input}/></label>
