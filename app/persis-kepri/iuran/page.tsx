...
export default async function MemberDues({searchParams}:{searchParams:Promise<{page?:string}>}){
  ...
  return <div lang="id" className="min-h-screen bg-slate-50 px-5 py-10 text-slate-900 sm:px-8"><div className="mx-auto max-w-6xl">
    ...
    {!user?<section className="mt-8 max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 sm:p-8"><h2 className="text-2xl font-bold">Masuk untuk mencatat iuran</h2><p className="mt-3 leading-7 text-slate-600">Gunakan akun website yang sudah dimiliki, atau buat akun untuk pencatatan iuran. Riwayat dan bukti pembayaran hanya tersedia bagi pemilik akun dan bendahara.</p><div className="mt-6 flex flex-wrap gap-3"><Link href="/login?next=%2Fpersis-kepri%2Fiuran" className="rounded-lg bg-emerald-800 px-5 py-3 font-bold text-white">Masuk</Link><Link href="/persis-kepri/pendaftaran" className="rounded-lg border border-emerald-800 px-5 py-3 font-bold text-emerald-900">Daftar sebagai anggota</Link></div></section>:<>
      ...
      <Link href="/persis-kepri/pendaftaran" className="mb-6 mr-3 inline-block rounded-lg bg-emerald-800 px-5 py-3 font-semibold text-white">Daftarkan data / lihat status anggota</Link>
