...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
      <nav className="mx-auto flex min-h-20 max-w-[96rem] items-center justify-between gap-4 px-5 py-3 sm:px-6 lg:px-8">
        <Link href="/" className="shrink-0" onClick={() => setOpen(false)}>
          <p className="font-academic text-lg font-bold tracking-tight text-slate-950 sm:text-xl 2xl:text-2xl">
            Dr. Hos Arie Sibarani
          </p>
          <p className="mt-1 hidden text-[8px] uppercase tracking-[0.18em] text-amber-700 md:block 2xl:text-[9px]">
        ...
        <div className="hidden items-center gap-4 2xl:flex">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className={`text-[13px] font-medium text-slate-700 transition hover:text-amber-700 ${
                item.name === "Raja Ali Haji Institute" ||
                item.name === "Constitutional Justice"
        ...
        <button
          ...
          className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-300 text-slate-900 transition hover:bg-slate-100 2xl:hidden"
        >
      ...
      <div
        ...
        className={`overflow-hidden border-t border-slate-200 bg-white transition-all duration-300 2xl:hidden ${
          open ? "max-h-[950px]" : "max-h-0 border-t-transparent"
        }`}
      >
